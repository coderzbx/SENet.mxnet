...
def main():
    ...
    if args.data_type == "cifar10":
        ...
        units = per_unit * 3
        symbol = resnext(units=units, num_stage=3, filter_list=filter_list, ratio_list=ratio_list,
                         num_class=args.num_classes, num_group=args.num_group,
        ...
        else [64, 64, 128, 256, 512], ratio_list=ratio_list, num_class=args.num_classes,
                         num_group=args.num_group, data_type="imagenet", drop_out=args.drop_out, bottle_neck=True
        ...
        symbol = resnext(units=units, num_stage=4, filter_list=[64, 256, 512, 1024, 2048] if args.depth >= 50
                         ...
                         num_group=args.num_group, data_type="kd", drop_out=args.drop_out, bottle_neck=True
            if args.depth >= 50 else False, bn_mom=args.bn_mom, workspace=args.workspace,
                         memonger=args.memonger)
    else:
    ...
    if not os.path.exists("/data/deeplearning/lane_detect/kd.senet.assist/output/models"):
        os.mkdir("/data/deeplearning/lane_detect/kd.senet.assist/output/models")
    model_prefix = "/data/deeplearning/lane_detect/kd.senet.assist/output/models/se-resnext-{}-{}-{}".format(
    ...
    model = mx.model.FeedForward(
        ...
        lr_scheduler=multi_factor_scheduler(begin_epoch, epoch_size, step=[100, 150, 200, 250, 300, 400], factor=0.1),
    )
    model.fit(
        X=train,
        eval_data=val,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', type=str, default='/data/deeplearning/lane_detect/kd.senet.assist/data/',
                        help='the input data directory')
    parser.add_argument('--data-type', type=str, default='kd', help='the dataset type')
    parser.add_argument('--list_dir', type=str, default='/data/deeplearning/lane_detect/kd.senet.assist/data/',
    ...
    parser.add_argument('--num-examples', type=int, default=85000, help='the number of training examples')
    parser.add_argument('--kv-store', type=str, default='device', help='the kvstore type')
    parser.add_argument('--model-load-epoch', type=int, default=0,
                        help='load the model on an epoch using the model-load-prefix')
    parser.add_argument('--frequent', type=int, default=20, help='frequency of logging')
    ...
    #     '/data/deeplearning/lane_detect/kd.senet.assist/log/log-se-resnext-{}-{}.log'.format(args.data_type,
    #                                                                                          args.depth))
