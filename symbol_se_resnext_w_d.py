...
def residual_unit(data, num_filter, ratio, stride, dim_match, name, num_group, bottle_neck=True, bn_mom=0.9,
...
def resnext(units, num_stage, filter_list, ratio_list, num_class, num_group, data_type, drop_out, bottle_neck=True,
    ...
    for i in range(num_stage):
        ...
        for j in range(units[i] - 1):
            body = residual_unit(body, filter_list[i + 1], ratio_list[2], (1, 1), True,
                                 name='stage%d_unit%d' % (i + 1, j + 2),
                                 num_group=num_group, bottle_neck=bottle_neck, bn_mom=bn_mom, workspace=workspace,
                                 memonger=memonger)
